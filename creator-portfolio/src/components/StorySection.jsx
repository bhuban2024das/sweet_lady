  ...
  return (
    <motion.div 
      ...
      style={{
        ...
        padding: '20px',
        pointerEvents: 'none' // Let clicks pass through the transparent part if needed
      }}
    >
      <div style={{ 
        position: 'relative', 
        ...
        borderRadius: '24px', 
        overflow: 'hidden',
        pointerEvents: 'auto', // Re-enable clicks just for the card
        ...
        <div style={{ position: 'absolute', top: 35, left: 15, zIndex: 10, display: 'flex', alignItems: 'center', gap: 10, color: '#fff' }}>
          <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=200&auto=format&fit=crop" style={{ width: 32, height: 32, borderRadius: '50%' }} />
          <span style={{ fontWeight: 600, fontSize: '0.9rem', textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>Rupkotha</span>
...
export default function StorySection() {
  ...
  return (
    <section style={{ padding: '20px 5%', maxWidth: '1200px', margin: '0 auto', display: 'flex', gap: '20px', overflowX: 'auto' }} className="no-scrollbar">
      ...
      <AnimatePresence>
        {isViewerOpen && (
          <StoryViewer stories={MOCK_STORIES} onClose={() => setIsViewerOpen(false)} />
        )}
      </AnimatePresence>
